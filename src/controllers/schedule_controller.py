...
class ScheduleController:
    ...
    Methods:
        - get_schedule_by_specialty(): Get schedule for a specialty
    ...
    def get_schedule_by_specialty(self, specialty_id: int) -> Schedule:
        """
        Get schedule for a specialty.
        
        Args:
            specialty_id: ID of the specialty
            
        Returns:
            Schedule: The schedule for the specialty
        """
        ...
        if not schedule:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No schedule found for specialty {specialty.name}"
            )
    ...
    def create_schedule(
        ...
        """
        Create a new schedule for a specialty.
    ...
    def get_teacher_schedule(self, teacher_id: int) -> Dict[str, Any]:
        ...
        for day in days:
            ...
            specialty = self.session.get(Specialty, schedule.specialty_id) if schedule else None
