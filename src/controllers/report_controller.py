...
class ReportController:
    ...
    def _create_pdf(self, report: Report, data: Dict[str, Any], file_path: str):
        ...
        for specialty in data.get("specialties", []):
            elements.append(Paragraph(f"Specialty: {specialty.get('name', 'Unknown')}", heading_style))
            
            # Students
            students = specialty.get("students", [])
            if students:
    ...
    def _create_excel(self, report: Report, data: Dict[str, Any], file_path: str):
        ...
        headers = ['Specialty', 'Student', 'Email', 'Enrollments']
        ...
        row = 2
        for specialty in data.get("specialties", []):
            for student in specialty.get("students", []):
                ws_students.cell(row=row, column=1, value=specialty.get('name', 'Unknown'))
        ...
        headers = ['Specialty', 'Student', 'Module', 'Date', 'Status']
        for col, h in enumerate(headers, 1):
            ...
            for student in specialty.get("students", []):
                for enrollment in student.get("enrollments", []):
                    for record in enrollment.get("attendance_records", []):
                        ws_att.cell(row=row, column=1, value=specialty.get('name', 'Unknown'))
